...
@rem *
@rem * build.bat
@rem *   Batch file used to build Fatio with MSVC 2019, 2022, 2026
@rem *
@rem ******************************************************************************
...
:END
TITLE Building Fatio with MSVC - Finished!
ENDLOCAL
EXIT /B
...
:SUBMSVC
ECHO.
TITLE Building Fatio with MSVC - %~1 "%~2|%~3"...
CD /D %~dp0
"MSBuild.exe" /nologo fatio.sln /target:fatio;%~1 /property:Configuration=%~2;Platform=%~3^ /consoleloggerparameters:Verbosity=minimal /maxcpucount /nodeReuse:true
...
TITLE %~nx0
ECHO. & ECHO.
ECHO Usage: %~nx0 [Clean^|Build^|Rebuild] [Win32^|x64^|all^|No32bit] [Debug^|Release^|LLVMDebug^|LLVMRelease^|all]
ECHO.
